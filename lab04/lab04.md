...
#### Демонстрация работы
Запустим скрипт, перейдём по существующему адресу и несуществуеющему адресу и посмотрим, что выведется в ```logs.txt``` 
(для того, чтобы что-то вывелось в файл нужно послать программе сигнал SIGINT (ctrl+c или кнопка стоп в pycharm)
и обновить страницу в браузере, тогда программа завершается корректно и выводит всё в файл. У меня так 
в коде происходит, потому что идёт постоянное прослушивание канала и ожидание запроса, вот после запроса уже легко
останавливается код).
...
Несуществующий сайт(сработал обработчик ошибок):
...
#### Демонстрация работы
Запустим скрипт, запустим дважды сайт, я вывожу в консоль, что всё в порядке.
![4.jpg](screenshoots%2F4.jpg)

Можно посмотреть и как выглядит получившейся кеш:
...
Вот мы переходим по сайту и выводим ошибку 502

![7.jpg](screenshoots%2F7.jpg)

Хотя можно убедиться, что сайт такой есть:

![7_1.jpg](screenshoots%2F7_1.jpg)

Если же перейти по несуществующему сайту, 
то тоже получим ошибку 502, потому что я решил, 
что незачем даже пытаться обращаться к сайту, если он в блэклисте:

![8.jpg](screenshoots%2F8.jpg)
